...
pure integer function locate(x, xgrid)
...
pure real(8) function lininterp1(xgrid, ZZ, x)

	implicit none

	integer :: nx, xndx
	real(8) :: t, xz
    real(8), intent(in) :: xgrid(:), ZZ(:)
	real(8), intent(in) :: x !query point
...
real(8) function lininterp2(xgrid, ygrid, ZZ, x, y)

	implicit none

	integer :: nx, ny, xndx, yndx
	real(8) :: t, uu, xyz
    real(8), intent(in) :: xgrid(:), ygrid(:), ZZ(:,:)
    real(8), intent(in) :: x, y ! query points
